...
if __package__ in (None, ""):
    # When run as: python src/tools/backtracking.py
    # add src/ so `futoshiki` package is importable.
    sys.path.insert(0, str(Path(__file__).resolve().parents[1]))

from futoshiki.solvers.backtracking import backtracking_gen, backtracking_solve
...
__all__ = ["backtracking_gen", "backtracking_solve"]
